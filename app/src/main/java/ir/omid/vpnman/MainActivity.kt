...
class MainActivity : ComponentActivity() {
    ...
    private val vpnPermission = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == RESULT_OK) {
            pendingServer?.let(::startVpnService)
        } else {
            // User dismissed the system VPN consent dialog: nothing will ever start the
            // service now, so clear the optimistic "connecting" spinner shown on tap.
