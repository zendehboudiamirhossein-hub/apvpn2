...
class MyVpnService : VpnService() {
    ...
    // Set explicitly by stopVpn()/disconnect so the watchdog can tell an intentional
    // teardown apart from the core dying on its own (network drop, server hiccup, etc).
    @Volatile private var intentionalStop = false
    private var reconnectAttempts = 0

    private val baseRxBytes = TrafficStats.getUidRxBytes(Process.myUid()).coerceAtLeast(0)
    private val baseTxBytes = TrafficStats.getUidTxBytes(Process.myUid()).coerceAtLeast(0)
    ...
    /**
     * Polls the tunnel roughly once a second while connected: keeps the live data-usage
     * counters in VpnStateStore fresh, and detects the Xray core dying on its own (a
     * dropped mobile network, a server-side hiccup) so we can auto-reconnect instead of
     * silently leaving the user "connected" with a dead tunnel.
     */
    private fun startWatchdog() {
        ...
        watchdogJob = serviceScope.launch {
            ...
            while (true) {
                delay(1000)
                val rx = (TrafficStats.getUidRxBytes(Process.myUid()) - baseRxBytes).coerceAtLeast(0)
                val tx = (TrafficStats.getUidTxBytes(Process.myUid()) - baseTxBytes).coerceAtLeast(0)
                VpnStateStore.updateTraffic(rx, tx)

                if (coreController?.isRunning != true) {
                    missedBeats++
                    // Require a couple of consecutive misses before reacting, so a brief
                    // internal hiccup doesn't trigger an unnecessary reconnect.
                    if (missedBeats >= 2 && !intentionalStop) {
                        attemptReconnect()
                        return@launch
                    }
                } else {
    ...
    private suspend fun attemptReconnect() {
        ...
        if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
            VpnStateStore.update(ConnectionState.ERROR, currentName, "اتصال چند بار قطع شد. لطفاً دوباره تلاش کنید")
            stopForeground(STOP_FOREGROUND_REMOVE)
            stopSelf()
            return
        }
        reconnectAttempts++
        VpnStateStore.update(ConnectionState.RECONNECTING, currentName)
        val nm = getSystemService(NotificationManager::class.java)
        nm.notify(NOTIFICATION_ID, buildNotification("اتصال قطع شد؛ در حال تلاش دوباره…"))
        // Backoff: 2s, 4s, 8s...
        delay(2000L * (1 shl (reconnectAttempts - 1)))
        if (intentionalStop) return
        startVpn(currentConfig, currentName)
    }
